...
export async function POST(request) {
  ...
  if (!apiKey) {
    return NextResponse.json(
      {
        ok: false,
        message: "OPENAI_API_KEY is not set. Add it to .env.local to generate reviews.",
      },
      { status: 503 },
    );
  }
  ...
  if (!businessId || typeof businessId !== "string") {
    return NextResponse.json(
      { ok: false, message: "businessId required." },
      { status: 400 },
    );
  }
  ...
  if (!Number.isInteger(stars) || stars < 3 || stars > 5) {
    return NextResponse.json(
      { ok: false, message: "Rating must be 3, 4, or 5." },
      { status: 400 },
    );
  }
  ...
  const { data: biz, error } = await admin
    .from("businesses")
    .select("name, address, gmb_link, keywords, products")
    .eq("id", businessId)
    .maybeSingle();
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You only output valid JSON with a single key `reviews` whose value is an array of exactly 3 non-empty strings.",
        },
        {
          role: "user",
    ...
    if (!raw) {
      return NextResponse.json(
        { ok: false, message: "Empty model response." },
        { status: 502 },
      );
    }
    ...
    if (reviews.length < 3) {
      return NextResponse.json(
        { ok: false, message: "Model returned fewer than 3 reviews." },
        { status: 502 },
      );
    }
