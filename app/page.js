    ...
    <div className="flex min-h-full flex-1 flex-col">
      <header className="sticky top-0 z-20 border-b border-white/5 bg-navy/90 backdrop-blur-md">
        <div className="mx-auto flex w-full max-w-5xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          ...
          <nav className="flex items-center gap-2 sm:gap-3">
            <Link
              href="/login"
              className="rounded-full border border-white/10 px-3 py-2 text-sm font-medium text-text-muted transition hover:border-white/20 hover:text-white sm:px-4"
            >
              Owner login
            </Link>
            <Link
              href="/login"
              className="rounded-full bg-accent px-3 py-2 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(255,59,92,0.25)] transition hover:brightness-110 sm:px-4"
            >
              Get started
            </Link>
          </nav>
      ...
      <main className="flex flex-1 flex-col">
        <section className="relative overflow-hidden px-4 pb-16 pt-10 sm:px-6 sm:pb-24 sm:pt-16">
          <div
            aria-hidden
            className="pointer-events-none absolute -right-24 top-0 h-72 w-72 rounded-full bg-accent/20 blur-3xl sm:-right-10 sm:h-96 sm:w-96"
          />
          <div
            aria-hidden
          ...
          <div className="relative mx-auto flex w-full max-w-5xl flex-col gap-10 lg:flex-row lg:items-center lg:gap-16">
            <div className="flex-1 space-y-6">
              ...
              <h1 className="text-balance text-3xl font-bold leading-tight tracking-tight text-white sm:text-4xl sm:leading-tight lg:text-5xl lg:leading-[1.1]">
                ...
                <span className="text-accent">Google reviews</span> — without
                the awkward ask.
              </h1>
              <p className="max-w-xl text-pretty text-base leading-relaxed text-text-muted sm:text-lg">
                InsightRep is an AI-assisted QR journey: your team gets a
                branded flow, guests get three ready-to-post lines, and your
                listing gets consistent, keyword-aware reviews that still feel
                real.
              </p>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <Link
                  href="/login"
                  className="inline-flex h-12 items-center justify-center rounded-full bg-accent px-6 text-sm font-semibold text-white shadow-[0_12px_40px_rgba(255,59,92,0.3)] transition hover:brightness-110"
                >
                  Start as a business owner
                </Link>
                <p className="text-center text-xs text-text-muted sm:text-left sm:text-sm">
                  No card to explore the owner dashboard. Customer flow opens
                  from your QR.
                </p>
              </div>
              <dl className="grid grid-cols-3 gap-3 pt-2 sm:max-w-md sm:gap-4">
                {[
                  ...
                  <div
                    key={item.k}
                    className="rounded-xl border border-white/10 bg-navy-muted/60 px-3 py-3 text-center sm:px-4"
                  >
                    <dt className="text-[10px] font-medium uppercase tracking-wide text-text-muted sm:text-xs">
                      {item.k}
                    </dt>
                ...
                <div className="space-y-3 rounded-xl bg-navy/80 p-4 ring-1 ring-white/5">
                  ...
                  <div className="flex justify-center gap-2 py-2">
                    {[3, 4, 5].map((n) => (
                      <span
                        key={n}
                        className={`flex h-11 min-w-11 items-center justify-center rounded-full px-3 text-sm font-semibold tabular-nums ${
                          n === 5
                            ? "bg-accent text-white shadow-lg shadow-accent/30"
                            : "border border-white/15 bg-white/5 text-text-muted"
                        }`}
                      >
                        {n}★
                      </span>
                  ...
                  <p className="text-center text-[11px] text-text-muted">
                    Only 3–5★ flows — we keep the experience positive.
                  </p>
                  <div className="space-y-2 border-t border-white/10 pt-3">
                    <p className="text-xs font-medium text-text-muted">
                      Quick questions
                    </p>
                    {["Food quality", "Service", "Would recommend"].map(
                      (q) => (
                        <div
                          key={q}
                          className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2 text-xs text-white/90"
                        >
                          <span>{q}</span>
                  ...
                  <div className="rounded-lg border border-dashed border-accent/40 bg-accent/5 p-3">
                    ...
                    <p className="mt-1 text-xs leading-relaxed text-text-muted">
                      “Warm service, standout butter chicken, will book the
                      terrace again…”
                    </p>
                  </div>
                  <div className="flex gap-2 pt-1">
                    <span className="flex-1 rounded-lg bg-white/10 py-2 text-center text-xs font-medium text-white/80">
                      Copy review
                    </span>
                    <span className="flex-1 rounded-lg bg-accent py-2 text-center text-xs font-semibold text-white">
                      Open Google
                    </span>
                  </div>
                </div>
          ...
          <div className="mx-auto max-w-5xl">
            <div className="mb-10 max-w-2xl">
              ...
              <p className="mt-3 text-base text-text-muted sm:text-lg">
                Owners control the story; customers do the typing in seconds —
                tuned for busy Indian outlets and front-of-house teams.
              </p>
            </div>
            <div className="grid gap-8 lg:grid-cols-2">
              <div className="rounded-2xl border border-white/10 bg-navy p-5 sm:p-6">
                <div className="mb-5 flex items-center gap-2">
                  <span className="rounded-lg bg-accent/15 px-2 py-1 text-xs font-semibold text-accent">
                    Owner
                  </span>
                ...
                <ol className="space-y-4">
                  {stepsOwner.map((s, i) => (
                    <li key={s.title} className="flex gap-3">
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/10 text-sm font-bold text-accent">
                        {i + 1}
                      </span>
                      <div>
              ...
              <div className="rounded-2xl border border-white/10 bg-navy p-5 sm:p-6">
                <div className="mb-5 flex items-center gap-2">
                  <span className="rounded-lg bg-white/10 px-2 py-1 text-xs font-semibold text-white">
                    Customer
                  </span>
                  <h3 className="text-lg font-semibold text-white">
                    Scan → post
                ...
                <p className="mt-6 text-sm text-text-muted">
                  Each business gets a unique path:{" "}
        ...
        <section className="px-4 py-14 sm:px-6 sm:py-20">
          <div className="mx-auto max-w-5xl">
            <h2 className="text-center text-2xl font-bold text-white sm:text-3xl">
              Why teams pick InsightRep
            </h2>
            <ul className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {[
                {
                  title: "India-first OTP",
                  body: "MSG91-backed login — the same pattern staff already trust on Zomato and bank apps.",
                },
                {
                  title: "SEO-aware drafts",
                  body: "Keywords and featured products steer the model without stuffing or sounding robotic.",
                },
                {
                  title: "Mobile-first QR",
                  body: "Large tap targets, short steps, and copy-paste into Google in one motion.",
                },
              ].map((f) => (
                <li
                  key={f.title}
                  className="rounded-2xl border border-white/10 bg-navy-muted/40 p-5 transition hover:border-accent/30"
                >
        ...
        <section className="border-t border-white/5 px-4 py-14 sm:px-6 sm:py-20">
          <div className="mx-auto flex max-w-5xl flex-col items-center gap-6 rounded-2xl border border-accent/25 bg-gradient-to-br from-accent/10 via-navy-muted/50 to-navy p-8 text-center sm:p-12">
            <h2 className="max-w-xl text-2xl font-bold text-white sm:text-3xl">
              Ready to fill your Google listing with real guest voices?
            </h2>
            <p className="max-w-lg text-sm text-text-muted sm:text-base">
              Create your profile, drop a QR on tables or the reception desk,
              and let InsightRep handle the wording.
            </p>
            <Link
              href="/login"
              className="inline-flex h-12 items-center justify-center rounded-full bg-accent px-8 text-sm font-semibold text-white shadow-[0_12px_40px_rgba(255,59,92,0.35)] transition hover:brightness-110"
            >
