...
function BusinessLoadingSkeleton() {
  return (
    <div className="flex flex-col items-center gap-0 pt-10 pb-6 px-4">
      <style>{`
        @keyframes ir-shimmer { 0%,100%{opacity:0.35}50%{opacity:0.75} }
      ...
      <div className="ir-sk" style={{ height:28, width:200, borderRadius:10, marginBottom:10, animationDelay:'0.05s' }} />
      <div className="ir-sk" style={{ height:13, width:160, borderRadius:6, marginBottom:40, animationDelay:'0.1s' }} />
      <div className="ir-sk" style={{ height:18, width:180, borderRadius:6, marginBottom:8, alignSelf:'flex-start', animationDelay:'0.15s' }} />
      ...
      <div className="ir-sk" style={{ height:48, width:'100%', borderRadius:50, marginBottom:12, animationDelay:'0.2s' }} />
    </div>
  );
}
...
function RatingStep({ rating, hoverRating, setRating, setHoverRating, onContinue, businessId, businessType }) {
  ...
  return (
    <section className="mt-10 space-y-4">
      ...
      {isNegative && (
        <NegativeFeedbackCard businessId={businessId} rating={rating} businessType={businessType} />
      )}
...
function MoodStep({ mood, setMood, onContinue, onBack, busy, limitReached, genError, customNote, setCustomNote, availableMoods }) {
  return (
    <section className="mt-10 space-y-6">
          ...
          return (
            <button key={m.id} type="button" onClick={() => setMood(m.id)}
              className={"relative flex flex-col items-start gap-1.5 rounded-2xl border p-4 text-left transition-all duration-200 " +
                (selected ? "border-accent bg-accent/10 scale-[1.02]" : "border-white/10 bg-navy-muted/40 hover:border-white/25 hover:bg-white/5")}>
              {selected && (
      ...
      <div className="space-y-2">
        <label className="text-sm text-text-muted">Anything specific to mention? <span className="text-white/30">(Optional)</span></label>
        <textarea value={customNote} onChange={(e) => setCustomNote(e.target.value)}
          placeholder="e.g. The pasta was amazing, staff was very welcoming…" rows={3} maxLength={300}
      ...
      <button type="button" disabled={busy || limitReached || !mood} onClick={onContinue}
...
export default function CustomerReviewPage() {
  ...
  const prefetchResult = useRef(null);
    ...
    async function load() {
      ...
      try {
        ...
        if (lastReview) {
          ...
          if (daysSince < 30) { setStep("already_reviewed"); return; }
        }
        setStep("rating");
  ...
  async function generate() {
    ...
    try {
      ...
      if (!hasNote && prefetchResult.current?.ok && Array.isArray(prefetchResult.current?.reviews)) {
        data = prefetchResult.current; prefetchResult.current = null;
      } else {
      ...
      setGenError(err?.name === "AbortError" ? "Taking longer than usual — please try again." : "Something went wrong. Please try again.");
      setStep("mood");
  ...
  async function copyAndOpen() {
    ...
    try { localStorage.setItem(`ir_reviewed_${businessId}`, Date.now().toString()); } catch {}
  ...
  if (step === "already_reviewed") return (
    <div className="min-h-[100dvh] bg-navy px-4 py-16 flex flex-col items-center justify-center text-center gap-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-text-muted max-w-xs mx-auto">You already left a review for this place recently. We appreciate your support!</p>
      </div>
      <div className="rounded-2xl border border-white/10 bg-navy-muted/40 p-5 max-w-xs w-full space-y-2">
        ...
        <p className="text-xs text-text-muted leading-relaxed">To keep Google reviews authentic, we limit one review per device every 30 days.</p>
        <button type="button" onClick={() => { try { localStorage.removeItem(`ir_reviewed_${business?.id}`); } catch {} setStep("rating"); }}
      ...
      <div className="mx-auto max-w-lg">
        ...
        {step === "rating" && (
          <RatingStep rating={rating} hoverRating={hoverRating} setRating={setRating}
            setHoverRating={setHoverRating} onContinue={() => setStep("mood")}
            businessId={businessId} businessType={business?.business_type} />
        )}
        ...
        {step === "success" && (
          <section className="mt-10 flex flex-col items-center gap-5 text-center">
            ...
            <div className="w-full rounded-2xl border border-white/10 bg-navy-muted/40 p-4 text-left space-y-3">
              ...
              <div className="space-y-2">
                {["Google will open — tap the star rating you want", "Tap the review text box — long press and Paste", "Hit Post — done in 10 seconds!"].map((txt, i) => (
